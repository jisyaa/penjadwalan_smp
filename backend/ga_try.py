...
def build_sessions(tables, target_hours_per_class=40):
    ...
    for _, r in pairs.iterrows():
        ...
        if jam == 1:
            ...
            if nama_mapel.upper() == "PJOK":
                # PJOK wajib 3 jam utuh
                sessions.append(
                    Session(sid, id_kelas, nama_kelas, id_mapel, nama_mapel, 3, None)
                )
                sid += 1
            else:
                # opsi fleksibel: 3 jam ATAU 2+1
                # kita buat 2 sesi split_group yang sama
                sessions.append(
                    Session(sid, id_kelas, nama_kelas, id_mapel, nama_mapel, 2, split_gid)
                )
                sid += 1
                sessions.append(
                    Session(sid, id_kelas, nama_kelas, id_mapel, nama_mapel, 1, split_gid)
                )
                sid += 1
                split_gid += 1
        elif jam == 4:
